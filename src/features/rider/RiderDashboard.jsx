...
export default function RiderDashboard() {
  ...
  const fetchMyTask = async () => {
    try {
      ...
      const allMine = res.data.data.filter(d => d.rider?._id === user._id);
      
      const activeMine = allMine.filter(d => d.status !== 'delivered');
      setTask(activeMine.length > 0 ? activeMine[0] : null);

      const pastMine = allMine.filter(d => d.status === 'delivered');
      setHistory(pastMine.reverse()); // latest first
    } catch (err) {
      console.error(err);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-24">
             ...
             <div className="p-4 border-t border-slate-200 bg-slate-50">
               <div className="flex items-center gap-3 px-2 py-2 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold truncate text-slate-900">{user?.name || 'Agent'}</p>
                    <p className="text-xs text-slate-500 truncate">Agent Identity</p>
                  </div>
               </div>
               <Button onClick={logout} className="w-full justify-start bg-white text-red-600 hover:bg-red-50 hover:text-red-700 border border-slate-200 shadow-sm text-sm font-medium rounded-lg h-10 px-3">
                  <SignOut size={18} className="mr-2 text-red-400" /> End Shift (Sign Out)
               </Button>
             </div>
          </div>
        </div>
      ...
      <main className="px-4 max-w-xl mx-auto w-full pt-6 sm:pt-10">
        ...
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
           ...
           <div className="flex gap-1.5 bg-slate-200/50 p-1.5 rounded-lg shrink-0">
              <button 
                onClick={() => setActiveTab('active')} 
                className={`px-4 py-1.5 text-[13px] font-bold rounded-md transition-all shadow-sm ${activeTab === 'active' ? 'bg-white text-slate-900 border border-slate-200/60' : 'text-slate-500 hover:text-slate-700 shadow-none'}`}
              >
                Active Route
              </button>
              <button 
                onClick={() => setActiveTab('history')} 
                className={`px-4 py-1.5 text-[13px] font-bold rounded-md transition-all shadow-sm ${activeTab === 'history' ? 'bg-white text-slate-900 border border-slate-200/60' : 'text-slate-500 hover:text-slate-700 shadow-none'}`}
              >
        ...
        {activeTab === 'history' ? (
           <div className="space-y-4">
              {history.length === 0 ? (
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-10 text-center flex flex-col items-center">
                  <div className="h-12 w-12 bg-slate-50 border border-slate-100 rounded-xl flex items-center justify-center mb-4">
                     <CheckCircle size={24} weight="duotone" className="text-slate-400" />
                  </div>
                  <h3 className="text-base font-bold text-slate-900 mb-1">No Completed Tasks</h3>
                  <p className="text-slate-500 text-sm max-w-[250px]">You haven't completed any deliveries yet. They will appear here once finalized.</p>
                </div>
              ) : (
                history.map(pastTask => (
                  <div key={pastTask._id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                     <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        <div className="flex items-center gap-2">
                           <div className="h-8 w-8 bg-emerald-50 text-emerald-600 rounded-md flex items-center justify-center">
                             <CheckCircle size={18} weight="fill" />
                           </div>
                           <div>
                             <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-0.5">COMPLETED</p>
                             <p className="text-xs font-mono font-medium text-slate-800 leading-none">{pastTask.trackingId}</p>
                           </div>
                        </div>
                     </div>
                     <div className="p-5 flex flex-col gap-4">
                        <div className="relative pl-6">
                           <div className="absolute left-[3px] top-1.5 w-1.5 h-1.5 rounded-full bg-slate-300"></div>
                           <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Origin</p>
                           <p className="text-sm font-medium text-slate-700 leading-snug">{pastTask.pickupLocation?.address || pastTask.pickupAddress || 'Unknown Origin'}</p>
                        </div>
                        <div className="relative pl-6">
                           <div className="absolute left-[3px] top-1.5 w-1.5 h-1.5 rounded-full bg-emerald-500"></div>
                           <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mb-0.5">Destination</p>
                           <p className="text-sm font-semibold text-slate-900 leading-snug">{pastTask.dropoffLocation?.address || pastTask.deliveryAddress || 'Unknown Destination'}</p>
                        </div>
                     </div>
                  </div>
                ))
           ...
           <>
            {!task ? (
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-10 text-center flex flex-col items-center mt-4">
                ...
                <p className="text-slate-500 text-sm max-w-[250px] mb-6">Stand by for dispatch. Routes assigned to you will appear here.</p>
                <Button onClick={fetchMyTask} variant="outline" className="h-9 text-sm font-medium bg-white text-slate-700 shadow-sm border-slate-200">
                   Refresh Directory
                </Button>
              </div>
            ) : (
